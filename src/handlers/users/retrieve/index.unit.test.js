...
describe('Handler - Users - Retrieve', () => {
  describe('When called with valid request object', () => {
    ...
    it('should set res.send with the object returned from retrieveUser', function () {
      ...
      assert(
        this.res.send.calledWithExactly(RETRIEVE_USER_RESPONSE_OBJECT),
      );
    });
  });
  describe('When the user cannot be found', () => {
    before(function () {
      ...
      this.revert = retrieve.__set__(
        'retrieveUser',
        retrieveUserStubs.notFoundError,
      );
      return retrieve(req, this.res)
        .then((res) => {
          this.result = res;
  ...
  describe('When retrieveUser throws an unexpected error', () => {
    before(function () {
      const req = {};
      this.res = generateResSpy();
      const retrieveUserStubs = generateRetrieveUserStubs();
      this.revert = retrieve.__set__(
        'retrieveUser',
        retrieveUserStubs.genericError,
      );
      return retrieve(req, this.res)
        .then((res) => {
          this.result = res;
