...
describe('Create User Request Handler', () => {
  ...
  describe('When called with an invalid request object', () => {
    before(function () {
      ...
      this.res = generateResSpy();
      const createUserStubs = generateCreateUserStubs();
      this.revert = create.__set__(
        'createUser',
        createUserStubs.validationError,
      );
      return create(req, this.res)
        .then((res) => {
          this.result = res;
    ...
    it('should set res.send with the message of the validation error', function () {
      ...
      assert(
        this.res.json.calledWithExactly(
          match.has('message', CREATE_USER_VALIDATION_ERROR_MESSAGE),
        ),
      );
    });
  });
  describe('When createUser throws an unexpected error', () => {
    before(function () {
      const req = {};
      this.res = generateResSpy();
      const createUserStubs = generateCreateUserStubs();
