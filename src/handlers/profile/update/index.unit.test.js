...
describe('Handler - Profile - Update', () => {
  ...
  describe('When called with an invalid request object', () => {
    before(function () {
      ...
      this.revert = update.__set__(
        'updateProfile',
        updateProfileStubs.validationError,
      );
      return update(req, this.res)
        .then((res) => {
          this.result = res;
    ...
    it('should set res.send with the message of the validation error', function () {
      ...
      assert(
        this.res.json.calledWithExactly(
          match.has('message', UPDATE_PROFILE_VALIDATION_ERROR_MESSAGE),
        ),
      );
    });
  });

  describe('When updateProfile throws a User Not Found error', () => {
    before(function () {
      const req = {};
      this.res = generateResSpy();
      const updateProfileStubs = generateUpdateProfileStubs();
      this.revert = update.__set__(
        'updateProfile',
        updateProfileStubs.notFoundError,
      );
      return update(req, this.res)
        .then((res) => {
          this.result = res;
  ...
  describe('When updateProfile throws an unexpected error', () => {
    before(function () {
      const req = {};
      this.res = generateResSpy();
      const updateProfileStubs = generateUpdateProfileStubs();
      this.revert = update.__set__(
        'updateProfile',
        updateProfileStubs.genericError,
      );
      return update(req, this.res)
        .then((res) => {
          this.result = res;
