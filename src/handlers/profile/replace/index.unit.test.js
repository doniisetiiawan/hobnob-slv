...
describe('Handler - Profile - Replace', () => {
  describe('When called with valid request object', () => {
    before(function () {
      ...
      this.revert = replace.__set__(
        'replaceProfile',
        replaceProfileStubs.success,
      );
  ...
  describe('When called with an invalid request object', () => {
    before(function () {
      const req = {};
      this.res = generateResSpy();
      const replaceProfileStubs = generateReplaceProfileStubs();
      this.revert = replace.__set__(
        'replaceProfile',
        replaceProfileStubs.validationError,
      );
      return replace(req, this.res)
        .then((res) => {
          this.result = res;
    ...
    it('should set res with an application/json content-type header', function () {
      assert(this.res.set.calledOnce);
      assert(
        this.res.set.calledWithExactly('Content-Type', 'application/json'),
      );
    });
    ...
    it('should set res.send with the message of the validation error', function () {
      ...
      assert(
        this.res.json.calledWithExactly(
          match.has('message', REPLACE_PROFILE_VALIDATION_ERROR_MESSAGE),
        ),
      );
    });
  });

  describe('When replaceProfile throws a User Not Found error', () => {
    before(function () {
      const req = {};
      this.res = generateResSpy();
      const replaceProfileStubs = generateReplaceProfileStubs();
      this.revert = replace.__set__(
        'replaceProfile',
        replaceProfileStubs.notFoundError,
      );
      return replace(req, this.res)
        .then((res) => {
          this.result = res;
    ...
    it('should set res.json with a Not Found message', function () {
      assert(this.res.json.calledOnce);
      assert(
        this.res.json.calledWithExactly(match.has('message', 'Not Found')),
      );
    });
  });
  describe('When replaceProfile throws an unexpected error', () => {
    before(function () {
      const req = {};
      this.res = generateResSpy();
      const replaceProfileStubs = generateReplaceProfileStubs();
      this.revert = replace.__set__(
        'replaceProfile',
        replaceProfileStubs.genericError,
      );
      return replace(req, this.res)
        .then((res) => {
          this.result = res;
    ...
    it('should set res.json with a generic Internal Server Error message', function () {
      ...
      assert(
        this.res.json.calledWithExactly(
          match.has('message', 'Internal Server Error'),
        ),
      );
    });
  });
